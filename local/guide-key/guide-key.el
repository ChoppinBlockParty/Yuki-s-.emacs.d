...
is popped up at left or right."
  (let* ((scale (expt text-scale-mode-step text-scale-mode-amount))
         ...
         (len (length fkey-str-list))
         (width (min 100 (/ (frame-width) 2)))
         (cols
          (if horizontal
              (ceiling (/ (*  scale)
         ...
         (rows (+ (/ len cols) (if (= (mod len cols) 0) 0 1))))
    (setq cols (+ (/ len rows) (if (= (mod len rows) 0) 0 1)))
    ;; Insert following keys by columns per line.
    (loop for i from 0 to (1- len)
          do
          (let* ((col (mod i cols))
                 (row (/   i cols))
                 (str (nth (+ row (* col rows)) fkey-str-list)))
                 (insert (if str str "") (if (= col (1- cols)) "\n" " "))
                 )
          )
