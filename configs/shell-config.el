...
(defun my-shell ()
  ...
  (interactive)
  (let ((currentwin (get-buffer-window (current-buffer)))
        (newbuf     (generate-new-buffer-name "*shell*")))
    (generate-new-buffer newbuf)
    (set-window-dedicated-p currentwin nil)
    (set-window-buffer currentwin newbuf)
    (shell newbuf)
    )
  )
...
(defun my-shell-mode-hook ()
  ...
  (set-variable 'dirtrack-list '("[[:blank:][:cntrl:]]+[[:cntrl:]]\\[381m\\([^[:cntrl:]]+\\)[[:cntrl:]]\\[39m$" 1 nil))
